...
bool TiXmlDocument::LoadFile( const char* filename, TiXmlEncoding encoding )
{
	...
	if ( file )
	{
		...
		while(int byteRead=fread( buf,1,BUF_SIZE-1, file ) )
		{
			buf[byteRead]=0 ;
			char *src=buf ;
			char *dst=buf ;
...
TiXmlAttributeSet::~TiXmlAttributeSet()
{
	NAssert( sentinel.next == &sentinel );
	NAssert( sentinel.prev == &sentinel );
}


void TiXmlAttributeSet::Add( TiXmlAttribute* addMe )
{
	NAssert( !Find( addMe->Name() ) );	// Shouldn't be multiply adding to the set.
...
void TiXmlAttributeSet::Remove( TiXmlAttribute* removeMe )
{
	...
	NAssert( 0 );		// we tried to remove a non-linked attribute.
}
