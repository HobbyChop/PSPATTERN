...
// and set window size
	...
	// SetWindow caps the window at 36 cells but used to hand back
	// nothing, so a long message (a project name lands in one) left us
	// laying buttons out against a width the window never had -- they
	// drew off the right edge and the dialog lost its Yes.
...
void MessageBox::ProcessButtonMask(unsigned short mask,bool pressed) {
	// Every other modal ignores releases; this one did not, so a box
	// opened from a chord answered itself the moment the chord was
	// let go -- A was still in the mask on the way up.
	if (!pressed) return ;
	if (mask&EPBM_A) {
