...
SelectProjectDialog::SelectProjectDialog(View &view)
    : ModalView(view), content_(true), topIndex_(0), currentProject_(0),
      selected_(0) {}
...
void SelectProjectDialog::OnFocus() {
	...
	// lastProject_ is remembered across sessions and folders: clamp it
	// to what this folder actually holds, or the cursor points past the
	// end of the list and every action on it dereferences nothing
	currentProject_ = lastProject_;
	int size = content_.Size();
	if (currentProject_ >= size) {
		currentProject_ = (size > 0) ? size - 1 : 0;
	}
...
void SelectProjectDialog::ProcessButtonMask(unsigned short mask,bool pressed) {
    ...
    if (mask&EPBM_B) {
        ...
        if (mask & EPBM_A) {
            ...
            if (current != 0) {
                // The list contains ".." and any other folder that happens
                // to sit here (samplelib, stray directories). Deleting is
                // recursive, so aiming it at ".." would take out the parent
                // tree -- from the game folder that is PSP/GAME itself.
                // Only ever delete something that is actually a project.
                std::string name = current->GetName();
                std::string prefix = name.substr(0, 4);
                std::transform(prefix.begin(), prefix.end(), prefix.begin(),
                               ::tolower);
                if (name == ".." || prefix != "lgpt") {
                    View::SetNotification("only lgpt_ projects can be deleted");
                    return;
                }
                std::string message = "Delete project '" + name + "' ?";
                MessageBox *mb =
        ...
        if (mask & EPBM_A) {
			...
			case 0: // load
                ...
                for (it->Begin(); !it->IsDone(); it->Next()) {
					...
					// nothing under the cursor (empty folder, or a stale
					// remembered index): do nothing instead of
					// dereferencing null and dropping to the XMB
					if (current == 0) {
						return ;
					}
...
void SelectProjectDialog::warpToNextProject(int amount) {
    ...
    int size = content_.Size();
    if (size <= 0) {          // empty folder: nothing to move onto
        currentProject_ = 0;
        topIndex_ = 0;
        isDirty_ = true;
        return;
	...
	if (currentProject_<0) currentProject_=0 ;
	if (currentProject_>=size) currentProject_=size-1 ;
  ...
  if (dir) 
  {
		...
		for(it->Begin();!it->IsDone();it->Next())
    {
			...
			if (path.IsDirectory())
      {
				...
				// The program's own sample library sits beside the
				// projects and is never one. Listing it invited people
				// to open it and gave them nothing when they did.
				std::string lower=name ;
				std::transform(lower.begin(),lower.end(),lower.begin(),
				               ::tolower) ;
				bool ours=(lower=="samplelib") ;
				if ((name[0] != '.' || name[1]== '.') && !ours)
        {
