...
// Initialize exported variables
	 ...
	 // Defaults are "no envelope at all": attack and decay instant,
	 // sustain full, no release. At these values the envelope is
	 // skipped outright, so every sample written before this plays
	 // exactly as it did.
	 attack_=new Variable("attack",SIP_ATTACK,0x00) ;
	 decay_=new Variable("decay",SIP_DECAY,0x00) ;
	 sustain_=new Variable("sustain",SIP_SUSTAIN,0xFF) ;
	 release_=new Variable("release",SIP_RELEASE,0x00) ;
	 Insert(volume_) ;
	 Insert(attack_) ;
	 Insert(decay_) ;
	 Insert(sustain_) ;
...
void SampleInstrument::Stop(int channel) {
	...
	// Get Rendering params for current voice & fill init data
	 ...
	 renderParams *rp=renderParams_+channel ;
	 // With a release set the voice rings out instead of being cut;
	 // the channel keeps rendering while IsReleasing says so.
	 releaseAmpEnv(rp) ;
	 running_=false ;
}
...
/* The envelope runs in Q15 against FP_ONE. Times use the same curve
   as the synth's, so a value means roughly the same thing on both. */
void SampleInstrument::startAmpEnv(renderParams *rp) {

	int a=attack_->GetInt() ;
	int d=decay_->GetInt() ;
	int su=sustain_->GetInt() ;
  ...
  lastMidiNote_[channel]=rp->midiNote_ ;
	...
	// Process tick-level updates
	  ...
	  // Get additional parameters from variables
		...
		//
		// VOLUME 0x80 IS UNITY, NOT 0xFF.
		//
		// It used to be volume/255, so maximum was unity and there was
		// nowhere above it. That is fine for a wav somebody recorded hot,
		// and wrong for the baked kit, which is peak-normalised by
		// construction: a drum arrives at 0.97 of full scale, loses 3dB to
		// the centre pan law like everything else, and that is the loudest
		// it can ever be. Anything else on the channel can reach further,
		// so every track ended up with the drums pinned at 0xFF just to
		// keep up -- and a control with one usable value is not a control.
		//
		// Dividing by 128 instead puts unity at the DEFAULT and leaves a
		// clean 6dB above it. Quiet source material can be lifted where
      ...
      if (*rpFinished) {
			  ...
			  if (rpKrateCount--==0) {
					...
					if (hasUpdaters) {
						...
						fpattenuate=fp_mul(rp->attenuate_,attnscale) ;
						// No 'fixed' here: these have to ASSIGN to the
						// coefficients the sample loop multiplies by,
						// further out. Declaring them shadowed those
						// and threw the new values away at the end of
						// the block, so the panner updater moved
						// rp->pan_ and nothing downstream ever read it
						// -- PAN_ on a sampler channel did nothing at
						// all, and a whole mix of panned drums
...
/*HOG:5*/				*fltSpeedPtr = fp_add(*fltSpeedPtr,fp_mul(difr,fltParm1)); //mul by cutoff, less cutoff = no sound, so it's better not be 0.
				...
				// amplitude envelope, per sample so a fast attack
				// does not zipper
				if (rp->envActive_) {
					if (rp->envStep_>0) {
						rp->envLevel_+=rp->envStep_ ;
						if (rp->envLevel_>=rp->envTarget_) {
							rp->envLevel_=rp->envTarget_ ;
							rp->envStep_=0 ;
							// end of the attack arms the decay
					...
					s2=fp_mul(s2,rp->envLevel_) ;
					t2=fp_mul(t2,rp->envLevel_) ;
				}
