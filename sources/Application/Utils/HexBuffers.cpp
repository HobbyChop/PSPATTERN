...
void restoreHexBuffer(TiXmlNode *node,unsigned char *destination,unsigned dstSize) {

	unsigned char *dst=destination ;
	unsigned char *end=destination+dstSize ;
	...
	while(current) {
		...
		if (s) {
			...
			if (length>0) {
				if ((unsigned)length>(unsigned)(end-dst)) {
					length=(int)(end-dst) ;
				}
				if (length>0) {
			...
			if (textNode) {
				...
				for (unsigned int i=0;i<strlen(buffer)/2;i++) {
					if (dst>=end) break ;   // longer chunk than we can hold
