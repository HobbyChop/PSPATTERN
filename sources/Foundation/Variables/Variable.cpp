...
void Variable::SetString(const char *input, bool notify) {
    ...
    case CHAR_LIST: {
        ...
        if (match >= 0) {
            ...
            // Nothing matched. Hold on to the name so the next save writes
            // it back unchanged -- otherwise opening a project whose
            // samples/ folder is missing and saving it once destroys every
            // instrument's binding permanently.
            unmatched_ = input;
            value_.index_ = unmatchedIndex_;
        }
        break;
    }
    }
...
const char *Variable::GetString() {
    ...
    case CHAR_LIST:
        if (!unmatched_.empty()) {
            return unmatched_.c_str();   // round-trip what we could not resolve
        }
